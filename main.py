...
def index():
    ...
    print(user_agent)
    if user_agent != "co.median.android.awpbew":
...
def print_request():
    ...
    if type_batch == "Year wise":
        ...
        f_name = f"dat/{datetime.now().strftime('%H:%M')}_Overall_Attendance_Report_{f_date.date().strftime('%d-%m')}_to_{t_date.date().strftime('%d-%m')}.csv"
...
def only_det_report(cc):
    ...
    try:
        ...
        f_name = f"dat/{datetime.now().strftime('%H:%M')}_{sb_name}_{yr}_Detailed_Attendance_Report.csv"
...
def get_att_year():
    ...
    cc = []
    pc = []
    date_temp = datetime.now().date()  # .strftime("%d-%m-%Y")
    print_data_request = request.get_json()
    batch = print_data_request["batch"]
...
if __name__ == "__main__":
    ...
    print("i dont want to get executed")
